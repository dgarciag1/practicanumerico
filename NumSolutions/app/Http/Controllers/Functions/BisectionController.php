...
class BisectionController extends Controller
{
    ...
    {
        return view('user.functions.bisection.values');
    }
    ...
    public function results(Request $request)
    {
        ...
        $command = 'python "'.public_path().'\methods\bisection.py" '."{$function} {$aPoint} {$bPoint} {$tolerance} {$iterations}";
        exec($command, $output);
        $data["title"] = __('functions.bisection.results');
        $data["results"] = $output;
        $data["error"] = false;
        ...
        return view('user.functions.bisection.results')->with("data",$data);
