...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            {{ __('arrays.names.partial') }}
                <a class="btn btn-outline-info return-btn" href="{{ route('arrays.partial.initial') }}"><img src="{{ asset('/icons/arrow-return-left.svg') }}" class="delete-icon"></a>
        </div>
        <div class="card-body">
            @if($errors->any())
            <ul id="errors">
            ...
            @endif
            <form method="POST" action="{{ route('arrays.partial.results') }}">
                @csrf
                <div class="card">
                    <div class="card-header">
