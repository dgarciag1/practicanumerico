...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            {{ __('arrays.names.sor') }}
                <a class="btn btn-outline-info return-btn" href="{{ route('arrays.sor.initial') }}"><img src="{{ asset('/icons/arrow-return-left.svg') }}" class="delete-icon"></a>
        </div>
        <div class="card-body">
            @if($errors->any())
            <ul id="errors">
            ...
            @endif
            <form method="POST" action="{{ route('arrays.sor.results') }}">
                @csrf
                <div class="card">
                    <div class="card-header">
                    ...
                    <div class="card-body">
                        <div class="form-row">  
                            <div class="form-group col-md-6">
                                <label>{{ __('arrays.sor.tolerance') }}</label>
                                <input type="text" class="form-control" placeholder="{{ __('arrays.holder.tolerance') }}" name="tolerance" value="{{ old('tolerance') }}" />
                            </div>
                            <div class="form-group col-md-6">
                                <label>{{ __('arrays.sor.w_value') }}</label>
                                <input type="text" class="form-control" placeholder="{{ __('arrays.holder.w_value') }}" name="w_value" value="{{ old('w_value') }}" />
                            </div>
                            <div class="form-group col-md-6">
                                <label>{{ __('arrays.sor.iterations') }}</label>
