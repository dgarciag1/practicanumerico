...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            {{ __('interpolation.names.lagrange') }}
                <a class="btn btn-outline-info return-btn" href="{{ route('interpolation.lagrange.initial') }}"><img src="{{ asset('/icons/arrow-return-left.svg') }}" class="delete-icon"></a>
        </div>
        <div class="card-body">
            @if($errors->any())
            <ul id="errors">
            ...
            @endif
            <form method="POST" action="{{ route('interpolation.lagrange.results') }}">
                @csrf
                <div class="card">
                    <div class="card-header">
